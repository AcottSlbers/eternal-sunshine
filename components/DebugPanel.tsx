      ...
      <section className="mt-6" aria-labelledby="visual-score-debug">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          {ranking.results.map((item) => (
            <article key={item.camera.id} className="rounded-xl border border-white/[0.08] bg-white/[0.025] p-4">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-stone-200">{item.camera.name}</p><p className="mt-1 text-xs text-stone-600">{item.visualScoreStatus}</p></div>
              ...
              <dl className="mt-4 grid grid-cols-2 gap-x-5 gap-y-1 border-t border-white/[0.06] pt-3 text-xs sm:grid-cols-3">
