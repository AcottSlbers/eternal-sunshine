...
interface CapabilityStats { total: number; withProviderUrl: number; withLivePlayer: number; snapshotOnly: number }
interface CoverageReport {
  ...
  capabilities: { candidates: CapabilityStats; gold: CapabilityStats };
...
export default async function CoveragePage() {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-7xl px-6 py-12 text-stone-200">
      ...
      <section className="mt-10 grid gap-3 sm:grid-cols-3 lg:grid-cols-4">{[
        ["Gold cameras", report.goldCount],
        ["Live-capable", `${report.capabilities.gold.withLivePlayer} (${Math.round(report.capabilities.gold.withLivePlayer / Math.max(1, report.goldCount) * 100)}%)`],
        ["Provider URL available", `${report.capabilities.gold.withProviderUrl} (${Math.round(report.capabilities.gold.withProviderUrl / Math.max(1, report.goldCount) * 100)}%)`],
        ["Snapshot-only", `${report.capabilities.gold.snapshotOnly} (${Math.round(report.capabilities.gold.snapshotOnly / Math.max(1, report.goldCount) * 100)}%)`],
        ["Candidates", report.candidateCount], ["Countries", report.countries], ["Known directions", report.knownDirections], ["Gold quality avg", report.goldQuality.average],
      ...
      <p className="mt-4 text-xs text-stone-500">Candidate pool: {report.capabilities.candidates.withLivePlayer} live · {report.capabilities.candidates.withProviderUrl} with provider URL · {report.capabilities.candidates.snapshotOnly} snapshot-only</p>
